...
$rowDetailPhoto = $d->rawQuery("select photo, id_parent, id from #_gallery where id_color = ? and id_parent = ? and com = ? and type = ? and kind = ? and val = ?", array($id_color, $id_pro, 'product', $type, 'man', $type));
$rowDetail = $d->rawQueryOne("select name$lang, photo from #_product where id = ? and type = ? limit 0,1", array($id_pro, $type));
?>
