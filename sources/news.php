...
if ($id != '') {
    /* Lấy bài viết detail */
    $rowDetail = $d->rawQueryOne("select id, view, file_attach,namefile, date_created, id_list, id_cat, id_item, id_sub, type, name$lang, slugvi, slugen, desc$lang, content$lang, photo, options from #_news where id = ? and type = ? and find_in_set('hienthi',status) limit 0,1", array($id, $type));
