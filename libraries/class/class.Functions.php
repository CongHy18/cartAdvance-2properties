...
class Functions
{
    ...
    /*My Markdown */
    public function mymarkdown($path = '', $params = array())
    {
        global $config;
    ...
    public function isYoutube($str)
    {
        ...
        elseif (preg_match('/https?:\/\/(www\.)?youtube\.com\/shorts\/[0-9a-zA-Z\-\_]+/i', $str)) {
            return true;
        } else {
            return false;
        }
    ...
    public function getImage($data = array())
    {
        ...
        if (!empty($info['sizes'])) {
            ...
            $ratio = explode("x", $info['size-error']); 
            $width = $ratio[0]; 
            $height = $ratio[1]; 
            $stye= 'style="display: inline-block;aspect-ratio: '.$width.'/'.$height.';object-fit: contain;" width="'.$width.'" height="'.$height.'"';
        }
     ...
     if ($info['isPicture']) {
        ...
        if (!empty($info['url'])) {
            $info['pathSrc1'] = $info['url'];
            $info['pathSrc2'] = $info['url'];
        } else {
            if (!empty($info['size-src'])) {
                $thumb_1 = intval($width/1.5).'x'.intval($height/1.5).'x'.intval($typeThumb).'/'. $info['upload'] . $info['image'];
                $info['pathSrc1'] = (!empty($info['isWatermark']) && !empty($info['prefix'])) ? ASSET . $info['watermark'] . "/" . $info['prefix'] . "/" . $thumb_1 : ASSET . $info['thumbs'] . "/" . $thumb_1;
                if ($info['isWebp']) {
                    $info['pathSrc1'] .= '.webp';
                }

                $thumb_2 = intval($width/2).'x'.intval($height/2).'x'.intval($typeThumb).'/'. $info['upload'] . $info['image'];
                $info['pathSrc2'] = (!empty($info['isWatermark']) && !empty($info['prefix'])) ? ASSET . $info['watermark'] . "/" . $info['prefix'] . "/" . $thumb_2 : ASSET . $info['thumbs'] ."/" . $thumb_2;
                if ($info['isWebp']) {
                    $info['pathSrc2'] .= '.webp';
                }
            } else {
                $info['pathSrc1'] = ASSET . $info['pathOrigin'];
                $info['pathSrc2'] = ASSET . $info['pathOrigin'];
            }
        }
        
        $result = "<picture>";
        $result .= '<source media="(min-width: 575px) and (max-width: 991px)" srcset="' . $info['pathSrc1']. '" />';
        $result .= '<source media="(min-width: 0px) and (max-width: 575px)" srcset="' . $info['pathSrc2'] . '" />';
        $result .= "<img " . $info['class'] . " " . $info['id'] .  $info['src'] . " ".@$stye." alt='" . $info['alt'] . "'  />";
        $result .= "</picture>";
    }else{
        /* Image */
        $result = "<img " . $info['class'] . " " . $info['id'] . " onerror=\"this.src='" . $info['pathError'] . "';\" " . $info['src'] . " ".@$stye." alt='" . $info['alt'] . "'  />";
    }
        return $result;
    }
    /* Get list gallery */
    ...
    public function pagination($totalq = 0, $perPage = 10, $page = 1, $url = '?')
    {
        ...
        $total = $totalq;
        $adjacents = "2";
        $firstlabel = "<i class='fa-thin fa-angles-left'></i>";
        $prevlabel = "<i class='fa-thin fa-angle-left'></i>";
        $nextlabel = "<i class='fa-thin fa-angle-right'></i>";
        $lastlabel = "<i class='fa-thin fa-angles-right'></i>";
        $page = ($page == 0 ? 1 : $page);
    ...
    // Webp Convert
    public function convertWebp($in)
    {
        global $config;
        ...
        $source = $_SERVER['DOCUMENT_ROOT'] . $config['database']['url'] . str_replace(ASSET, "", $in);
    ...
    public function createThumb($width_thumb = 0, $height_thumb = 0, $zoom_crop = '1', $src = '', $watermark = null, $path = THUMBS, $preview = false, $args = array(), $quality = 100)
    {
        ...
        if ($webp) {
            $this->convertWebp($upload_dir . $image_name);
        }
    ...
    public function getColor($id = 0)
    {
        ...
        $str = '<select data-id_parent="' . $id . '" data-type="' . $type . '"  id="dataColor" name="dataColor[]" class="select multiselect multiselect-cart-advanced" multiple="multiple" >';
    ...
    public function getSize($id = 0)
    {
        ...
        $str = '<select data-id_parent="' . $id . '" data-type="' . $type . '" id="dataSize" name="dataSize[]" class="select multiselect multiselect-cart-advanced" multiple="multiple" >';
    ...
    /*Get poduct cat*/
    public function getProductCat($pid = 0)
    {
        $row = null;
        if ($pid) {
            $row = $this->d->rawQuery("select * from #_product_cat where id_list = ? and find_in_set('noibat',status) and find_in_set('hienthi',status) order by numb,id desc", array($pid));
        }
        return $row;
    }

     /*Get post*/
    public function getPostInListNb($pid = 0, $table = '', $type = '',$limit = '')
    {
        $row = null;
        if ($pid) {
            $row = $this->d->rawQuery("select * from #_$table where id_list = ? and type = ? and find_in_set('noibat',status) and find_in_set('hienthi',status) order by numb,id desc $limit", array($pid,$type));
        }
        return $row;
    }
    public function getProSale($pid = 0)
    {
        $row = null;
        if ($pid) {
            $row = $this->d->rawQueryOne("select * from #_product_sale where id_parent = ? order by id asc limit 0,1", array($pid));      
        }
        return $row;
    }

    /* Get product */
    public function getProductItem($product = array())
    {
        $str = '';
        ...
        if (!empty($product)) {
            ...
            /* Get markdown */
            $str = $this->markdown('product/product', $params);
        }
        return $str;
    }

    /* Get product */
    public function getProductSale($product_sale = array())
    {
        $str = '';

        if (!empty($product_sale)) {
            /* Data */
            $product_sale['target'] = 'target="_blank"';
            $params = array();
            $params['sale'] = $product_sale;
            /* Get markdown */
            $str = $this->markdown('product/form-price', $params);
        }
        return $str;
    }
