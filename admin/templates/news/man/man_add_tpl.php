    ...
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
                                        ...
                                        <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if (isset($config['news'][$type]['desc']) && $config['news'][$type]['desc'] == true) { ?>
                                                <div class="form-group">
                                                    ...
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['news'][$type]['desc_cke']) && $config['news'][$type]['desc_cke'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[desc<?= $k ?>]" id="desc<?= $k ?>" rows="5" placeholder="Mô tả (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('desc' . $k)) ?: $func->decodeHtmlChars(@$item['desc' . $k]) ?></textarea>
                                                </div>
                                            <?php } ?>
                                            <?php if (isset($config['news'][$type]['content']) && $config['news'][$type]['content'] == true) { ?>
                                                <div class="form-group">
                                                    <label for="content<?= $k ?>">Nội dung (<?= $k ?>):</label>
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['news'][$type]['content_cke']) && $config['news'][$type]['content_cke'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[content<?= $k ?>]" id="content<?= $k ?>" rows="5" placeholder="Nội dung (<?= $k ?>)"><?= $func->decodeHtmlChars($flash->get('content' . $k)) ?: $func->decodeHtmlChars(@$item['content' . $k]) ?></textarea>
                                                </div>
                                            <?php } ?>
                                        </div>
                                    <?php } ?>
                                    <?php if (isset($config['news'][$type]['link_iframe']) && $config['news'][$type]['link_iframe'] == true) { ?>
                                        <div class="form-group">
                                            <label for="link_iframe<?= $k ?>">Tọa độ google map iframe: <a href="https://www.google.com/maps" target="_blank">(Lấy mã nhúng)</a></label>
                                            <textarea class="form-control  text-sm " name="data[link_iframe]" id="link_iframe" rows="5" placeholder="Tọa độ google map iframe"><?= $func->decodeHtmlChars($flash->get('link_iframe')) ?: $func->decodeHtmlChars(@$item['link_iframe']) ?></textarea>
                                        </div>
                                    <?php } ?>
                                </div>
                            </div>
                ...
                <?php if (isset($config['news'][$type]['images2']) && $config['news'][$type]['images2'] == true) { ?>
                    <div class="card card-primary card-outline text-sm">
                        ...
                        <div class="card-body">
                            ...
                            /* Photo detail */
                            $photo2Detail = array();
                            $photo2Detail['upload'] = UPLOAD_NEWS_L;
                            $photo2Detail['image'] = (!empty($item) && $act != 'copy') ? $item['photo2'] : '';
                            $photo2Detail['dimension'] = "Width: " . $config['news'][$type]['width2'] . " px - Height: " . $config['news'][$type]['height2'] . " px (" . $config['news'][$type]['img_type'] . ")";
                ...
                <?php if (isset($config['news'][$type]['file']) && $config['news'][$type]['file'] == true) { ?>
                    <div class="form-group">
                        <div class="upload-file mb-2">
                            <p>Upload tập tin:</p>
                            <input type="text" style="max-width: 300px;" class="form-control for-seo text-sm mb-2" name="data[namefile]" id="namefile" placeholder="Tiêu đề file" value="<?= (!empty($flash->has('namefile'))) ? $flash->get('namefile') : @$item['namefile'] ?>">
                            ...
                            <?php } ?>
                            <strong class="d-block text-sm"><?= $config['news'][$type]['file_type'] ?></strong>
                        </div>
                    </div>
                <?php } ?>
            </div>
        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <?php if (isset($config['news'][$type]['price_from']) && $config['news'][$type]['price_from'] == true) { ?>
                        <div class="form-group col-md-4">
                            <label class="d-block" for="price_from">Giá đầu:</label>
                            <div class="input-group">
                                <input type="text" class="form-control format-price price_from text-sm" name="data[price_from]" id="price_from" placeholder="Giá đầu" value="<?= (!empty($flash->has('price_from'))) ? $flash->get('price_from') : @$item['price_from'] ?>">
                    ...
                    <?php } ?>
                    <?php if (isset($config['news'][$type]['price_to']) && $config['news'][$type]['price_to'] == true) { ?>
                        <div class="form-group col-md-4">
                            <label class="d-block" for="price_to">Giá cuối:</label>
                            <div class="input-group">
                                <input type="text" class="form-control format-price price_to text-sm" name="data[price_to]" id="price_to" placeholder="Giá cuối" value="<?= (!empty($flash->has('price_to'))) ? $flash->get('price_to') : @$item['price_to'] ?>">
                                <div class="input-group-append">
                                    <div class="input-group-text"><strong>VNĐ</strong></div>
                                </div>
                            </div>
