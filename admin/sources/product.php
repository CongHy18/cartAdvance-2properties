...
function saveMan()
{
    ...
    $data_regular_price = (!empty($_POST['data_regular_price'])) ? $_POST['data_regular_price'] : 0;
    $data_sale_price = (!empty($_POST['data_sale_price'])) ? $_POST['data_sale_price'] : 0;
    $data_discount = (!empty($_POST['data_discount'])) ? $_POST['data_discount'] : 0;
    ...
    if ($data_regular_price) {
        $data_regular_price = (isset($data_regular_price) && $data_regular_price != '') ? str_replace(",", "", $data_regular_price) : 0;
    }
    if ($data_sale_price) {
        $data_sale_price = (isset($data_sale_price) && $data_sale_price != '') ? str_replace(",", "", $data_sale_price) : 0;
    }
    if ($data_discount) {
        $data_discount = (isset($data_discount) && $data_discount != '') ? $data_discount : 0;
    }
    /* Post seo */
    ...
    if ($id && $act != 'save_copy') {
        ...
        if ($d->update('product', $data)) {
            ...
            if ($func->hasFile("file2")) {
                ...
                if ($photo = $func->uploadImage("file2", $config['product'][$type]['img_type'], UPLOAD_PRODUCT, $file_name)) {
            ...
            if (!empty($config['product'][$type]['color']) || !empty($config['product'][$type]['size'])) {
                if (!empty($dataColor) && !empty($dataSize)) {
                    ...
                    $dataSale3 = array();
                    $dataSale3['id'] = 'regular_price';
                    $dataSale3['data'] = $data_regular_price;

                    $dataSale4 = array();
                    $dataSale4['id'] = 'sale_price';
                    $dataSale4['data'] = $data_sale_price;

                    $dataSale5 = array();
                    $dataSale5['id'] = 'discount';
                    $dataSale5['data'] = $data_discount;
                    ...
                    foreach ($dataSale1['data'] as $v_sale1) {
                        ...
                        if (!empty($dataSale2['data'])) {
                            foreach ($dataSale2['data'] as $v_sale2) {
                                ...
                                if (!empty($dataSale3['data'])) {
                                    $dataSale[$dataSale3['id']] = array_shift($dataSale3['data']);
                                }

                                if (!empty($dataSale4['data'])) {
                                    $dataSale[$dataSale4['id']] = array_shift($dataSale4['data']);
                                }

                                if (!empty($dataSale5['data'])) {
                                    $dataSale[$dataSale5['id']] = array_shift($dataSale5['data']);
                                }
        ...
        if ($d->insert('product', $data)) {
            ...
            if ($act == 'save_copy') {
                $func->transfer("Sao chép dữ liệu bị lỗi", "index.php?com=product&act=copy&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id, false);
            } else {
                $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=copy&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id, false);
            }
        }
    }
}
...
function deleteMan()
{
    ...
    if ($id) {
        /* Lấy dữ liệu */
        $row = $d->rawQueryOne("select id, photo, photo2 from #_product where id = ? and type = ? limit 0,1", array($id, $type));
...
function saveBrand()
{
    global $d, $curPage, $func, $flash, $config, $com, $type, $strUrl;
